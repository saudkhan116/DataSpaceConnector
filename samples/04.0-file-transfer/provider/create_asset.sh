...
echo ""
echo -e "\033[1mCreating ContractDefinition\033[0m"
echo "---------"

[ -z "$defaultDocument" ] && defaultDocument="test-document-recycler"
echo "Enter Asset Name (default '$defaultDocument')"
read -r document
[ -z "$document" ] && document=$defaultDocument
...
echo "---------"
curl -X POST "$providerDataMgmtUrl/$dataMgmtPath/contractdefinitions" --header "$apiKey: $apiKeyValue" --header "Content-Type: application/json" --data "$__payload" -w 'Response Code: %{http_code}\n'
echo ""
echo -e "\033[1mCreated ContractDefinition for the Asset\033[0m"
echo "---------"
# curl -X GET "$providerDataMgmtUrl/$dataMgmtPath/assets/$assetId" --header "$apiKey: $apiKeyValue" --header "Content-Type: application/json" | jq
curl -X GET "$providerDataMgmtUrl/$dataMgmtPath/contractdefinitions/$assetId" --header "$apiKey: $apiKeyValue" --header "Content-Type: application/json"
echo "---------"
